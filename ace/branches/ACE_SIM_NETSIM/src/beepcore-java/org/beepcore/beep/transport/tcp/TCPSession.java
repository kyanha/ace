...
public class TCPSession extends SessionImpl {
	...
	public static TCPSession createInitiator(Socket sock,
			ProfileRegistry registry, String servername) throws BEEPException {
	 ...
	 */
	public static TCPSession createInitiator(Socket sock,
			ProfileRegistry registry) throws BEEPException {
	...
	public String toString() {
		return super.toString() + " (" + socket.getLocalAddress() + ":"
	...
	public void terminateIO(boolean force) {
		...
		try {
			...
			log.debug("Waiting for Socket Sotimeoutinterrupted");
		}
	}
	...
	protected Session reset(SessionCredential localCred,
			SessionCredential peerCred, SessionTuningProperties tuning,
			ProfileRegistry reg, Object argument) throws BEEPException {
		if (log.isTraceEnabled()) {
			log.trace("Reset as " + (isInitiator() ? "INITIATOR" : "LISTENER"));
		}
		...
		Session newSession = new TCPSession(s, reg,
				(isInitiator() ? CHANNEL_START_ODD : CHANNEL_START_EVEN),
	...
	private boolean processCoreFrame(byte[] headerBuffer, int amountRead, InputStream is) 
		...
		headerFound: while (true) {
			...
			if (headerLength > Frame.MAX_HEADER_SIZE) {
				throw new BEEPException("Malformed BEEP header, no CRLF #5");
			}
	...
	private void processSEQFrame(byte[] headerBuffer, int amountRead, InputStream is) 
			...
			while (headerLength < amountRead) {
				if (headerBuffer[headerLength] == '\n') {
					if (headerLength == 0 || headerBuffer[headerLength - 1] != '\r') {
						...
						info += headerLength + ", ";
						info += (new String(headerBuffer)) + "]";
						throw new BEEPException("Malformed BEEP header #1 " + info);
					}
		...
		if (java.util.Arrays.equals(type, MESSAGE_TYPE_SEQ) == false) {
			...
			throw new BEEPException("Malformed BEEP header #2 [" + s + "]");
		}
		...
		if (header.hasMoreTokens()) {
			throw new BEEPException("Malformed BEEP Header #3");
		}
