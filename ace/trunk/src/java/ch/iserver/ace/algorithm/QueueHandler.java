...
public class QueueHandler extends Thread {
	...
	/**
	 * The priority which is given to remote operations when processing the
	 * buffers.
	 */
	...
	/**
	 * This object is used in conjunction with a notification mechanism with the
	 * two buffers <code>localOperationBuffer</code> and
	 * <code>remoteRequestBuffer</code>. When the queue handler realises that
	 * there are no more operations and requests to process, it waits on that
	 * object as follows:
	 * 
	 * <pre>
	 * synchObj.wait();
	 * </pre>
	 * 
	 * When one of the buffers turns non-empty again, it notifies the
	 * synchronization object by calling:
	 * 
	 * <pre>
	 * synchObj.notify();
	 * </pre>
	 * 
	 * Hence the queue handler returns from the wait state and can proceed its
	 * execution.
	 * 
	 ...
	 */
	public QueueHandler(Algorithm algo, SynchronizedQueue localOpBuf,
			SynchronizedQueue remoteReqBuf, SynchronizedQueue outReqBuf,
			Object synchObj) {
		this.algo = algo;
	...
	public void run() {
		...
		try {
			while (true) {
				...
				if (localOperationBuffer.isEmpty()
						&& remoteRequestBuffer.isEmpty()) {
