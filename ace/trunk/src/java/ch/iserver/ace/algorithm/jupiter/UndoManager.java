...
public class UndoManager {
	...
	private void clearRedo() {
		...
		if (!localUndoCandidates.isEmpty()
				&& nextUndoOperation < localUndoCandidates.size() - 1) {
			localUndoCandidates.subList(nextUndoOperation + 1,
					localUndoCandidates.size()).clear();
		}
	}
	...
	/**
	 * Adds a remote request, e.g. a request that has been received over the
	 * network.
	 * 
	 * @param request
	 *            the remote request
	 ...
	 * @return all operations with a remote operation count greater or equals to
	 *         <code>base</code>.
	 */
	public List getRemoteRequests(int base) {
		...
		while (iter.hasNext()) {
			...
			if (((JupiterVectorTime) req.getTimestamp())
					.getRemoteOperationCount() >= base) {
				tmp.add(req);
			}
		}
		return tmp;
	...
	/**
	 * Returns a list of Object[]{Request, index} which all have a
	 * <code>localOperationCount</code> greater or equals to <code>base</code>.
	 * 
	 * @param base
	 *            the base count
	...
	public List getLocalTransformationSet(int base) {
		...
		// TODO: this is a code duplication of
		// OutgoingQueue.getTransformationSet()...
		...
		if (reqs.size() <= 1) {
			...
			if (!reqs.isEmpty()) {
				result
						.add(new Object[] { (Request) reqs.get(0),
								new Integer(0) });
			}
		} else {
			int undos = 0, pairs = 0;
			...
			// operations list examples: {d,u,u,d,d} {d,d,u,u} {u,u,d}; u=undo,
			// d=do
			while (iter.hasNext()) {
			...
			while (iter.hasNext()) {
				...
				if (op.isUndo() && pairs > 0 && undos == 0) {
					...
					// TODO: this is can be improved, but we have to remember
					// the index of this wrap into the operation list, so 
					...
					result.add(new Object[] { r,
							new Integer(localRequests.size() - 1 - opIndex) });
					if (op.isUndo()) {
						undos--;
					}
				} else {
	...
	public boolean canRedo() {
		return (localUndoCandidates.size() > 0 
				&& nextUndoOperation < localUndoCandidates.size() - 1);
	}
