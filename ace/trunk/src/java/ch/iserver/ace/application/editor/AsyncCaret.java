 ...
 * modified. Typically this will happen on the event thread as a result of some
 * mouse or keyboard event. Updates can also occur from some other thread
 * mutating the document. There is a property <code>AsynchronousMovement</code>
 * that determines if the caret will move on asynchronous updates. The default
 * behavior is to <em>not</em> update on asynchronous updates. If asynchronous
 * updates are allowed, the update thread will fire the caret position change to
 * listeners asynchronously. The repaint of the new caret location will occur on
 * the event thread in any case, as calls to <code>modelToView</code> are only
 * safe on the event thread.
 * <p>
 ...
 * reimplementing the <a href="#getSelectionHighlighter">getSelectionHighlighter</a>
 * method.
 * <p>
 ...
 * @see Caret
 */
public class AsyncCaret extends Rectangle implements Caret, FocusListener,
...
//				}
	 ...
	 * 
	 * @return all of this caret's <code>ChangeListener</code>s or an empty
	 ...
	 * <code><em>Foo</em>Listener</code>s upon this caret.
	 ...
	 * example, you can query a <code>DefaultCaret</code> <code>c</code> for
	 * its change listeners with the following code:
	 * 
	 * <pre>
	 ...
	 * 
	 * @return the delay in milliseconds. If this is zero the caret will not
	 *         blink.
	...
	void handleSetDot(int dot, Position.Bias dotBias) {
		...
		mark = dot;
		if (this.dot != dot || this.dotBias != dotBias || selectionTag != null
...
//		}
	...
	private void updateSystemSelection() {
		if (this.dot != this.mark && component != null) {
			...
			if (clip != null) {
				clip.setContents(new StringSelection(component
								.getSelectedText()), getClipboardOwner());
				ownsSelection = true;
			}
		}
	}
	...
	/**
	 * This is invoked after the document changes to verify the current dot/mark
	 * is valid. We do this in case the <code>NavigationFilter</code> changed
	 * where to position the dot, that resulted in the current location being
	 * bogus.
	 */
	...
	/**
	 * This is used to indicate if the caret currently owns the selection. This
	 * is always false if the system does not support the system clipboard.
	 */
	private boolean ownsSelection;

	/**
	 * If this is true, the location of the dot is updated regardless of the
	 * current location. This is set in the DocumentListener such that even if
	 * the model location of dot hasn't changed (perhaps do to a forward delete)
	 * the visual location is updated.
	 */
	private boolean forceCaretPositionChange;

	/**
	 * Whether or not mouseReleased should adjust the caret and focus. This flag
	 * is set by mousePressed if it wanted to adjust the caret and focus but
	 * couldn't because of a possible DnD operation.
	 */
	...
	/**
	 * ClipboardOwner that will toggle the visibility of the selection when
	 * ownership is lost.
	 */
	...
	class UpdateHandler implements PropertyChangeListener, DocumentListener,
		...
		public void insertUpdate(DocumentEvent e) {
			if (async || SwingUtilities.isEventDispatchThread()) {
				...
				if (changed != 0) {
					Position.Bias dotBias = AsyncCaret.this.dotBias;
					if (dot == offset) {
						Document doc = component.getDocument();
						boolean isNewline;
						try {
		...
		public void removeUpdate(DocumentEvent e) {
			if (async || SwingUtilities.isEventDispatchThread()) {
				...
				if (newMark == newDot) {
					...
					Position.Bias dotBias = AsyncCaret.this.dotBias;
					Position.Bias markBias = AsyncCaret.this.markBias;
					if (adjustDotBias) {
