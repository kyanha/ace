...
which adds a single method \texttt{set\-Name} to the \texttt{Remote\-User}
...
The callback must also return a \texttt{Session\-Connection\-Callback} to the
network layer.
...
creates a request that can then be sent to the \texttt{Session\-Connection}.
...
\texttt{Acknowledge\-Strategy} objects are creates by corresponding factories.
The factory interface is \texttt{Acknowledge\-Strategy\-Factory}.
