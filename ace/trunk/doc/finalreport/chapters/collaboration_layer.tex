...
The \texttt{User\-Registry} manages the set of currently known users. The
...
\texttt{Join\-Network\-Callback}. The name of the implementation class
is \texttt{Join\-Network\-Callback\-Impl}.
...
\texttt{Configurable\-Session} interface. This interface has a methods to
set the \texttt{Session\-Connection} as well as the
\texttt{Participant\-Session\-Callback}.
...
The \texttt{Session\-Impl} then wraps the passed in reference to the
...
\texttt{Session\-Connection.Failure\-Handler} interface and
...
avoid that the sender's outgoing queue do not get too full. 
