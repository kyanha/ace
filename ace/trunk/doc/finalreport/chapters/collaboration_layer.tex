...
functionality have been thorougly explained in chapter 
\ref{chapter:archoverview}.
...
\texttt{Invitation\-Callback}, and \texttt{Service\-Failure\-Handler}. Figure \ref{fig:collaboration.collaborationserviceimpl} depicts this
situation.
...
set of currently known users and documents to objects implementing the
...
to that user's name. It also takes care that there is only one 
\texttt{Remote\-Document} for a unique document id.

The objects registered in the \texttt{User\-Registry} are of 
type \texttt{Mutable\-Remote\-User},
...
\label{sect:collaboration.joincallback}
To forward a join request to the network layer, the collaboration layer has
to implement the \texttt{Join\-Network\-Callback} interface
(\texttt{Join\-Network\-Callback\-Impl}). The
...
network layer. The following code snippet shows how the \texttt{accepted}
...
\small{\begin{verbatim}
  public SessionConnectionCallback accepted(SessionConnection connection) {
      ...
      session.setSessionCallback(getJoinCallback().accepted(session));
      return session;
  }
\end{verbatim}}

The \emph{getJoinCallback()} retrieves the \texttt{Join\-Callback} passed from
the application layer when initiating the join. The \texttt{Session\-Factory}
and the \texttt{Configurable\-Session} interfaces are described in the
next section.
...
\texttt{Session\-Factory} creates objects implementing the 
\texttt{Configurable\-Session} interface. 
...
the requests and passes the transformed operations to the
...
When a join request was accepted, the network layer passes a
\texttt{Session\-Connection} to the collaboration layer
(see section \ref{sect:collaboration.joincallback}). The connection
is passed to the implementation of the \texttt{Configurable\-Session}
interface, the \texttt{Session\-Impl}. This implementation
wraps the passed in reference to the
\texttt{Session\-Connection} with use of a \texttt{Thread\-Domain} (see
appendix \ref{chapter:threaddomains}). Therefore method invocations on the
wrapped \texttt{Session\-Connection} are executed asynchronously. 
...
object implementing the \texttt{Session\-Connection\-Wrapper.Failure\-Handler}
...
The calls from the dynamic proxy to the \texttt{Session\-Connection\-Wrapper}
are asynchronous. The methods of the \texttt{Session\-Connection\-Wrapper}
...
\texttt{handle\-Failure} on the failure handler. The
\texttt{Session\-Impl} implements the 
\texttt{Session\-Connection\-Wrapper.Failure\-Handler} interface and
...
see section \ref{sect:archoverview.acknowledgestrategies}.
The following code snippet shows the implementation of the 
\texttt{Acknowledge\-Action} used by the \texttt{Session\-Impl}

\small{\begin{verbatim}
  Timestamp timestamp = getAlgorithm().getTimestamp();
  int siteId = getAlgorithm().getSiteId();
  SessionConnection connection = getConnection();
  connection.sendAcknowledge(siteId, timestamp);
\end{verbatim}}
...
class, which implements the \texttt{Publisher\-Connection}
interface too (see figure \ref{fig:collaboration.publishedsession}).
...
used to send acknowledge messages (see section 
...
\texttt{Publisher\-Connection} (implemented by 
...
participants is kept up-to-date. Each session server runs inside the same JVM
as the publisher's application.
...
Incoming messages from a particular participant are coming through an
...
one particular participant into the session 
server. Outgoing communication happens 
...
there is a pair of \texttt{Participant\-Port} and 
\texttt{Participant\-Connection} objects.
...
The session server acts as a delivery station for messages from participants. 
...
participant. The generated request is then passed to a 
...
objects 1, 2, and 5 that are aligned vertically all process messages for/from
...
The processing in the session server from the point where a request crosses the
...
The \texttt{Server\-Logic} has methods that allows users to join,
...
The method \texttt{get\-Participant\-Id} is used to assign participant
ids. The implementation assigns a user that joins and then leaves the session 
...
The session server needs a solid failure handling, otherwise it can
easily happen that the document replicas start to diverge. The interface
\texttt{Failure\-Handler} is used to handle failures in the session server. 
...
The failure handler is passed to the following objects:
\begin{itemize}
...
The \texttt{Server\-Logic\-Impl} implements the \texttt{Failure\-Handler}
 interface. The 
...
one particular participant from the session. 
...
This exception is caught by the participant ports and the failure
handler is notified. This expels the participant from the session. 
First, a participant that sends invalid requests
...
the session server rejected the
corresponding request. Thus, the document state of
...
A \texttt{Forwarder} receives messages processed by
\texttt{Participant\-Port}s. For
...
\texttt{Forwarder} objects (implementation of \emph{Composite} pattern). 
This interface adds methods to add and
...
with that failure. The default forwarder used is described later in 
the section about the \texttt{Document\-Updater}.
...
a \texttt{Caret\-Update} passed to the \texttt{send\-Caret\-Update} method.
...
The \texttt{Participant\-Forwarder} also gets an \texttt{Acknowledge\-Strategy},
...
other participants can happen concurrently to processing
...
connection that has thrown an exception as no longer usable.
...
\subsection{Incoming Thread Domain}
\label{sect:collaboration.threaddomains}
As noted above, only one thread may access the session server transformation
section at a time. Now, instead of using synchronized statements or
...
all incoming references to the session server with a \texttt{Single\-Thread\-Domain}. This
...
grow endlessly (see section \ref{sect:algorithm.outgoingqueue}). 
To avoid this situation, a site should acknowledge from time to time the
messages it processed from the other site. 
Acknowledge messages are rather simple.
...
send an acknowledge message. It must be initialized with an
\texttt{Acknowledge\-Action} through the \texttt{init} method. The action's
\texttt{execute} method is invoked whenever the strategy thinks it is
...
Code that uses an \texttt{Acknowledge\-Strategy} should call
...
\subsubsection{Timed Strategy}
Another strategy could be based on a timer that for instance 
fires after 60 seconds during
which no messages have been sent. The \texttt{reset} method in that
case would simply restart the timer.
...
can be received within the timeout period.

\subsubsection{AcknowledgeStrategyFactory}
\texttt{Acknowledge\-Strategy} objects are created by corresponding factories.
The interface implemented by these factories is
...
that are used to notify others about the change of the local caret 
and to receive such changes from other users.
The two additional objects used are \texttt{Caret\-Update}
...
of the publisher's JVM. 
All incoming requests pass through a \texttt{Participant\-Port}, all
outgoing through a \texttt{Participant\-Connection}. So the session server
is totally agnostic about the fact that the publisher is running inside
the same JVM. Through the introduction of a few additional
interfaces it would be possible to host the session server logic on a different
computer. This is not supported at the moment by ACE.
