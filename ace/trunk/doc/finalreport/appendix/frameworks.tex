...
Filtering can either be static or dynamic. Static filtering can be used for example to filter out  elements with a special type. Assume the class \textit{User} from the example in section \ref{frameworks_glazed_eventlist} has an additional \texttt{age} property. It is a simple \textit{int} with the corresponding setter and getter methods.
