...
\subsection{Concepts}
BEEP is a peer-to-peer protocol in the sense that there is no notion of client or server. For convenience weÕll refer to the peer that starts a connection as the \emph{initiator}, and the peer accepting the connection as the \emph{listener}. When a connection is established between the two, a BEEP session is created.
...
\subsection{Introduction}
The core interface of \emph{Glazed Lists} (see \href{http://www.publicobject.com/glazedlists/}{http://www.publicobject.com/glazedlists/}) is the \texttt{Event\-List}. It extends the familiar \texttt{java.util.List} interface with the following extra features:

\begin{itemize}
 \item \emph{Event listeners:} fires events when it is modified
 \item \emph{Concurrency:} has locks so data can be shared between threads
\end{itemize}

Based on these basic features some very useful extensions are built:

\begin{itemize}
 \item lists that generate events whenever an element in the list changes
 \item sorted lists
 \item filtered lists
\end{itemize}

The following sections explain the basics of these features, why they are interesting for our application development and some simple code fragments that show their usage.

\subsection{EventList}
\label{frameworks_glazed_eventlist}
An \texttt{Event\-List} is an interface that extends \texttt{java.util.List}. It represents an observable list which makes it possible to add and remove \textit{List\-Event\-Listeners}. Further, there is a possibility to get a read/write lock to control concurrency access to the list.

An \texttt{Observable\-Element\-List} is a list that fires update events whenever a list element changes. Changes to list elements are detected by registering an appropriate listener on every list element. Whenever elements are added, a listener is registered on the element, and whenever elements are removed, the listeners are unregistered from the element. An \texttt{Observable\-Element\-List.Connector} needs to be specified in the constructor which defines the necessary logic for registering and unregistering an appropriate listener for detecting modifications on the list elements.
...
Assume a simple GUI component that displays the list of users in a \texttt{JList}. By wrapping an \texttt{Observable\-Element\-List} and an \texttt{Event\-List\-Model} around the list of users, the \texttt{JList} diplaying the users will be automatically updated whenever a user changes his name and will repaint the element with the new username.
...
A \texttt{Sorted\-List} is an \texttt{Event\-List} that sorts another \texttt{Event\-List}. The sorting strategy is specified with a \texttt{Comparator}. If no \texttt{Comparator} is specified, all of the elements of the source \texttt{Event\-List} must implement \texttt{Comparable}. The following example adds the \texttt{Comparable} interface to the user class from the example in section \ref{frameworks_glazed_eventlist} and creates a \texttt{Sorted\-List} out of the user list.
...
The \texttt{Filter\-List} filters another \texttt{Event\-List}. It is an \texttt{Event\-List} that shows a subset of the elements of the source list. This subset is composed of all elements of the source \texttt{Event\-List} that match the filter.

Filtering can either be static or dynamic. \textbf{Static filtering} can be used for example to filter out elements with a specific type. Assume that the class \texttt{User} from the example in section \ref{frameworks_glazed_eventlist} has an additional age property. It is a simple \texttt{int} with the corresponding setter and getter methods. For example, use the following code to filter out and display only the users older than 20 years:
\begin{verbatim}
...
\textbf{Dynamic filtering} can be used for example if the list of users should be filtered by user names and the filter can change. The following example uses a \texttt{JText\-Field} containing the filter string. The filtered user list contains only the users matching the entered text.

The \texttt{Text\-Filterator} is used to extract Strings from a list element (in that case \texttt{User} objects) that are used to match the list element against the given filter string (from the \texttt{JText\-Field}).
...
\subsection{Summary}
Using \emph{Glazed Lists} for GUI programming (or even other applications using lists of data) speeds up the implementation because you do not have to care about simple things like sorting, filtering or updating list elements. It is definitely worth to spend some time to learn the basics of using \texttt{Event\-Lists}.
