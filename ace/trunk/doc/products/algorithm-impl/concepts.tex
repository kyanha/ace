...
In example 1, we insert the inverse operation \texttt{Del[1,s]} immediately after the operation. The procedure of inserting the inverse operation is called \emph{mirroring}. In the next step, the inverse operation \textttDel[1,s]} is transformed against the operation \texttt{Ins[1,a]} from user $Y$, which results in the correct operation \texttt{Del[2,s]}. See figure \ref{fig:concepts.undobytransformation}.
